...
import sys
sys.path.insert(0, '../')
from utils.helper_vision import *
...
if args.method == 'base':
    filename = args.dataset + '-' + args.model + '-' + args.method + '-' + str(args.nepoch)
elif args.method == 'sampling':
    filename = args.dataset + '-' + args.model + '-' + args.method + '-' + str(args.sampling_nmodel) + '-' + str(args.nepoch)
elif args.method == 'dropout':
    filename = args.dataset + '-' + args.model + '-' + args.dropoutmethod + '-' + args.method + '-' + str(args.drp_nmodel) + '-' + str(args.drp_max_ratio)
elif args.method == 'awp':
    filename = args.dataset + '-' + args.model + '-' + args.method + '-' + args.awp_eps
...
if args.dataset == 'cifar10':
    datafolder = 'cifar10/data/'
    X_train, y_train, X_test, y_test = load_cifar10(args.dataroot, datafolder)
elif args.dataset == 'cifar100':
    datafolder = 'cifar100/data/'
    X_train, y_train, X_test, y_test = load_cifar100(args.dataroot, datafolder)
else:
    log.write('Dataset {} not supported\n'.format(args.dataset))
...
if args.method == 'base':
    model = fetch_model(log, args.model, output_dim, pre)
    train_start_time = time.localtime()
    model = train(log, args, trainloader, testloader, model, criterion, device)
    ...
    model = fetch_model(log, args.model, output_dim, pre)
    model.load_state_dict(torch.load(args.resultroot+args.dataset + '-' + args.model +'-base-' + str(args.nepoch) + '.pt'))
    ...
    log.write('Loading pre-train model: {}\n'.format(args.resultroot+args.dataset + '-' + args.model +'-base-' + str(args.nepoch) + '.pt'))
    ...
    base_model.load_state_dict(torch.load(args.resultroot+args.dataset+'-base.pt'))
